...
public class StudentServiceImp implements StudentService {
	...
	private void setStudent() {
		...
		if(Student.StudentInfo) {//학생 정보 저장소가 필요
	...
	private void addStudent() {
		...
		scan.nextLine();
		String info = scan.nextLine();
		//List<Student> studentList = new ArratList<Student>();학생 정보를 저장할 리스트 작성필요 Student.StudentInfo = 학생 클래스로 보낸 후 저장
		char isContinue = 'n';
		do {
		...
		if(Student == null) {//학생 정보 저장소를 만든 후 작성완료 : Student 작성
				System.out.println("등록된 학생 정보가 없습니다.");
				return;
		}
		if(Student.addStudent(num, name, major)) {
